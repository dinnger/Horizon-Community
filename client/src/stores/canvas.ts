...
const newStore = (name: string) => {
	return () => {
		...
		const initSubscriptionsExecution = ({ workflowId }: { workflowId: string }) => {
			...
			socketService.onWorkflowConsole(workflowId || '', (events: PanelConsole[]) => {
...
export const useCanvas = (uniqueStoreName: 'canvas' | 'execution' = 'canvas') => defineStore(uniqueStoreName, newStore(uniqueStoreName))()
